...
func TestWhere(t *testing.T) {
	...
	assert.Equal(t, "`a`.`id` BETWEEN ? AND ? AND b.name = ? AND `a`.`age` > ? AND `a`.`sex` IN (?,?,?) AND `b`.`status` IN (SELECT `user_id` FROM `user` WHERE `status` <> ?) AND `a`.`mail` IS NOT NULL AND `b`.`avatar` IS NULL AND `a`.`num` NOT BETWEEN ? AND ? AND `c`.`id` NOT IN (?,?) AND `c`.`test` NOT IN (SELECT `user_id` FROM `user` WHERE `status` <> ?) OR (`d`.`info` BETWEEN ? AND ? AND `d`.`other` like ?) AND (`d`.`test` IS NOT NULL AND `d`.`ss` <> ?)", h.Prepare())
	assert.Equal(t, []any{10, 20, "kovey", 100, 1, 2, 3, 1, 1000, 2000, 123, 45, 1, 100, 200, "%kk%", 1}, h.Binds())
	assert.Equal(t, "`a`.`id` BETWEEN ? AND ? AND b.name = ? AND `a`.`age` > ? AND `a`.`sex` IN (?,?,?) AND `b`.`status` IN (SELECT `user_id` FROM `user` WHERE `status` <> ?) AND `a`.`mail` IS NOT NULL AND `b`.`avatar` IS NULL AND `a`.`num` NOT BETWEEN ? AND ? AND `c`.`id` NOT IN (?,?) AND `c`.`test` NOT IN (SELECT `user_id` FROM `user` WHERE `status` <> ?) OR (`d`.`info` BETWEEN ? AND ? AND `d`.`other` like ?) AND (`d`.`test` IS NOT NULL AND `d`.`ss` <> ?)", h.Prepare())
}
