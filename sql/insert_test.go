...
func TestInsertFrom(t *testing.T) {
	...
	assert.Equal(t, "INSERT INTO `user` (`name`, `kovey`, `date`) SELECT `u`.`name`, `u`.`kovey`, `e`.`date` FROM `user_back` AS `u` INNER JOIN `email` AS `e` ON (`e`.`id` = `u`.`id`) WHERE `u`.`name` LIKE ?", in.Prepare())
