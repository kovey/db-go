...
func TestSelectPrepare(t *testing.T) {
	s := NewSelect("user", "u")
	s.Columns(meta.NewAliasColumn("id", "uid"), meta.NewAliasColumn("name", "username"), meta.NewAliasColumn("sex", "ss")).Limit(20).Offset(10).Order("create_time DESC", "id ASC").Group("id", "name")
	...
	j.Columns(meta.NewAliasColumn("name", "dname"), meta.NewAliasColumn("password", "password"))
	s.LeftJoin(j)
	j1 := NewJoin("cron", "c", "c.user_id=u.id")
	j1.Columns(meta.NewAliasColumn("time", "tt"), meta.NewAliasColumn("run", "runn"))
	s.RightJoin(j1)
	j2 := NewJoin("ext", "e", "e.user_id=u.id")
	j2.Columns(meta.NewAliasColumn("nickname", "nnn"), meta.NewAliasColumn("addr", "addr"))
